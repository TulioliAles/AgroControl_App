  ...
  template: `
    <section class="page-heading">
      <div>
        ...
        <p>Consulte os insumos cadastrados na API.</p>
      </div>
      ...
      @if (loading()) {
        ...
        <table mat-table [dataSource]="items()">
          ...
          <ng-container matColumnDef="commercialName">
            ...
            <td mat-cell *matCellDef="let item">{{ item.commercialName || '—' }}</td>
          </ng-container>
        ...
        @if (!items().length) {
          <div class="state"><mat-icon>inventory_2</mat-icon><span>Nenhum insumo cadastrado.</span></div>
        }
      }
    </mat-card>
  `,
...
export class AgriculturalInputListComponent {
  ...
  readonly columns = ['name', 'commercialName', 'status'];
  ...
  constructor() {
    ...
    this.http.get<PagedResult<AgriculturalInput>>(`${environment.apiUrl}/api/agricultural-inputs`, { params }).subscribe({
      next: (result) => { this.items.set(result.items); this.loading.set(false); },
      error: () => { this.error.set('Não foi possível carregar os insumos.'); this.loading.set(false); }
    });
  }
}
